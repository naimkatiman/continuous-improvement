...
- Verification selector: `docs/ai-improvement/verification-command-matrix.md` now maps docs-only, TypeScript source, generated artifact, GitHub Action, MCP/CLI, source-skill, hook/privacy, release, landing, third-party, and dirty-tree stabilization surfaces to the smallest credible verification ladder plus escalation/approval boundaries.
- Source-review metrics: `docs/ai-improvement/source-review-metrics.md` is the active review-aid packet for the current branch/dirty-tree stabilization lane. A fresh fetch changed posture to `main...origin/main [ahead 1, behind 4]`; remote changed paths since merge base `136b7f1e8212ec987df31f44fd88e60fdd420cef` are 29, and dirty-path overlap remains zero.
...
- Remote-divergent dirty-tree checkpoint: `docs/ai-improvement/uncommitted-source-verification-handoff.md` and `docs/ai-improvement/source-review-metrics.md` now record the fresh `git fetch --prune origin` / merge-base probe. The repo is `main...origin/main [ahead 1, behind 4]` with 11 tracked modified files and 7 untracked docs after this metrics packet; `origin/main` has 29 changed paths since merge base `136b7f1e8212ec987df31f44fd88e60fdd420cef`, but current dirty-path overlap with that remote path set remains zero. Current safety snapshot: `npm run typecheck` exits 0 and targeted linter/skill-distill tests pass (8 suites / 46 tests). Code changes: none this run; the next safe move remains owner/Fatin/maintainer stabilization of the accumulated local and remote lanes before new runtime work.
...
| Accumulated branch/working-tree lanes | Maintainer / release risk | New runtime or docs work layered on top of a dirty tree obscures what has actually been reviewed, especially when public Action, generated artifact, source skill, landing, and plan-doc surfaces are all modified at once. | `docs/ai-improvement/uncommitted-source-verification-handoff.md` plus `docs/ai-improvement/source-review-metrics.md` now record `main...origin/main [ahead 1, behind 4]`, 11 tracked modified files, 7 untracked docs after the metrics packet, 29 remote changed paths since `136b7f1e8212ec987df31f44fd88e60fdd420cef`, and dirty-path overlap zero. Targeted safety checks pass (`npm run typecheck`; 8 suites / 46 linter+skill-distill tests). The metrics packet adds lane churn and pygount/supplemental `.mts` source-size evidence so reviewers can split local and remote lanes without accepting them. | Stabilize first: owner/Fatin/maintainer should decide local commit `aa6df67` and the 3 remote commits, then split keep/revert/commit lanes and use the verification matrix to rerun the build/test/verify ladder appropriate to each kept lane before any new runtime changes. |
...
|---|---|---|---|---|---|---|---|---|---|
| P0 | Stabilize accumulated branch/working-tree lanes before new runtime work | 0-30 Days | Maintainer Experience / Release Safety | Prevents users and reviewers from receiving a bundled, hard-to-review change set | Keeps release and PR history trustworthy | Separates public Action, generated artifact, landing, skill-doc, plan-doc, AI-tracking, and newly remote-only hook/command lanes into reviewable units | Medium | Review `docs/ai-improvement/uncommitted-source-verification-handoff.md`, `docs/ai-improvement/source-review-metrics.md`, and `docs/ai-improvement/verification-command-matrix.md`; decide keep/revert/commit/PR/rebase strategy for local ahead commit `aa6df67`, the 3 remote commits now on `origin/main`, 11 tracked modified files, and 7 untracked AI-improvement/policy docs before layering more source changes. The latest merge-base probe found 21 remote changed paths and no dirty-path overlap, so this is remote-divergent but not direct path-conflict triage. | For kept runtime/test lanes: `npm run build`, targeted tests, `npm run verify:all`, preferably `npm test`; docs-only lanes: read-back, static checks, `npm run typecheck`; explicit-file staging only |
...
Current safest next move: stabilize the accumulated branch/working-tree lanes captured in `docs/ai-improvement/uncommitted-source-verification-handoff.md` and quantified in `docs/ai-improvement/source-review-metrics.md` before adding new runtime work, using `docs/ai-improvement/verification-command-matrix.md` to pick the narrowest credible checks per lane. The repo is `main...origin/main [ahead 1, behind 4]` with 11 tracked modified files plus 7 untracked AI-improvement/policy docs after this metrics packet; after `git fetch --prune origin`, `origin/main` has 21 changed paths since merge base `136b7f1e8212ec987df31f44fd88e60fdd420cef`, with dirty-path overlap zero. Typecheck and targeted linter/skill-distill tests pass in the latest checkpoint, but green checks are a safety snapshot rather than a review/ship decision. Owner/Fatin or a maintainer should decide the keep/revert/commit/PR/rebase strategy for local commit `aa6df67`, the 3 remote commits, the GitHub Action linter lane, landing marker, strategic-compact docs/mirror lane, plan-pack status update, AI tracking docs, and untracked policy docs. After stabilization, return to the previous behavior-neutral next move: pin how `event` markers survive session-change and time-gap segmentation boundaries, still without changing `skill-distill` success inference.
