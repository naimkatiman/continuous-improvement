...
export function extractRev(html) {
...
export async function waitForLiveRev(opts) {
    ...
    const start = now();
    let attempts = 0;
    let last = { ok: false, url: APEX_URL, live: null, attempts: 0 };
    while (true) {
        attempts += 1;
        for (const url of [APEX_URL, PAGES_DEV_URL]) {
            ...
            if (live !== null)
                last = { ok: false, url, live, attempts };
...
async function loadFetch(specifier) {
    if (!specifier)
        return defaultFetchHtml;
...
async function main() {
    ...
    if (!result.ok) {
        console.error(`::error::Landing deploy did not take: serving '${result.live ?? "<none>"}' but this release is ${expected}.`);
        exit(1);
    }
