...
- After any code change, run `npm run verify:all` (19 content invariants + typecheck: skill-mirror, skill-tiers, skill-law-tag, skill-count, skill-count-prose, command-count, test-count, invariant-count, docs-substrings, everything-mirror, routing-targets, superpowers-companion-table, doc-runtime-claims, test-imports-only, landing-version, scripts-citation-drift, third-party-shape, tool-count, reconcile-parity, typecheck). Anything below that is incomplete.
...
| 2026-07-10 | Marketplace lifecycle hooks and npx hook registrations still invoked Bash; Windows resolved WSL Bash, which could not open the native paths. The remaining 5-second host budget also cancelled valid cold Node starts under load. | Use Node entrypoints when Node is already required, migrate installer-owned legacy rows, and size host timeouts from observed cold-start latency. | `hook-runtime-portability.test.mts` + no-Bash, legacy-migration, and minimum-timeout cases. |
