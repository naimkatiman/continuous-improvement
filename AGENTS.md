...
| 2026-07-10 | Marketplace lifecycle hooks and npx hook registrations still invoked Bash; Windows resolved WSL Bash, which could not open the native paths. | Bash path validation was a compatibility patch. Use Node entrypoints when Node is already required, and migrate installer-owned legacy rows. | `hook-runtime-portability.test.mts` + no-Bash and legacy-migration installer cases. |
