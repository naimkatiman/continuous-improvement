...
For new users, the **file-based install path** is the recommended way to use PM-Skills. It is under active maintenance, ships the full current catalog of all 41 skills, and works with any agent supported by the open [`skills` CLI](https://github.com/vercel-labs/skills) or by Claude Code's slash-command system.
