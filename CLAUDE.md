...
- **recall Thai combining-mark fragmentation (LOW):** `/[^\p{L}\p{N}]+/u` treated Thai combining marks (Unicode `\p{M}`) as delimiters, fracturing Thai words into garbled length-2/3 fragments that recall then indexed. — **CLOSED in working tree**: both `recall-index` and `goal-state` now split on `/[^\p{L}\p{N}\p{M}]+/u`; regression tests in `src/test/recall-index.test.mts` and `src/test/goal-state.test.mts` pin Thai words with tone/vowel marks. Goal-state's 4-char floor now accepts length-4+ Thai keywords; the Korean short-word follow-up remains open.
