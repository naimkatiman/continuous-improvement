...
- Three known orphan `.mjs` files do NOT have `.mts` sources and are hand-authored: `bin/refresh-third-party.mjs`, `hooks/three-section-close.mjs`, `test/check-everything-mirror.test.mjs`. `bin/` and `test/` orphans are preserved by `npm run clean`; `hooks/` is not touched by clean. Treat as deferred follow-up #1.
