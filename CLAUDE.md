...
- Two known orphan `.mjs` files do NOT have `.mts` sources and are hand-authored: `bin/refresh-third-party.mjs` and `test/check-everything-mirror.test.mjs`. Both are preserved by `npm run clean` via an explicit orphan set in `package.json`. Treat as deferred follow-up #1.
