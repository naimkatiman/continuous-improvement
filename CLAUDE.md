...
- One known orphan `.mjs` file does NOT have a `.mts` source and is hand-authored: `bin/refresh-third-party.mjs`. It is preserved by `npm run clean` via an explicit orphan set in `package.json`. Treat as deferred follow-up #1.
