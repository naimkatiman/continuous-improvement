...
- `bin/refresh-third-party.mjs` and `test/check-everything-mirror.test.mjs` remain orphan `.mjs` files without `.mts` sources. `hooks/three-section-close.mjs` was resolved in this cycle.
