...
describe("config-guard-gate: classifyMutation", () => {
    it("flags Edit/Write/MultiEdit/NotebookEdit on a protected path", () => {
        ...
        assert.equal(classifyMutation("MultiEdit", { edits: [{ file_path: "README.md" }, { file_path: ".claude/settings.local.json" }] })?.target, ".claude/settings.local.json");
...
describe("hooks/config-guard.mjs (runtime PreToolUse hook)", () => {
    ...
    it("CI_CONFIG_GUARD_ALLOW=true lets one call through block mode and says so", () => {
        const r = runHook("Edit", { file_path: ".claude/settings.json", old_string: "a", new_string: "b" }, { CI_CONFIG_GUARD: "block", CI_CONFIG_GUARD_ALLOW: "true" });
        assert.equal(r.decision, "allow");
