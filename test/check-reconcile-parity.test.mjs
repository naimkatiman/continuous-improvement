...
import { checkDoc, checkRepo, fencedBlocks, normalizeForMatch, probeCommand, } from "../bin/check-reconcile-parity.mjs";
...
describe("normalizeForMatch", () => {
    it("strips shell quoting so '@{u}' and @{u} compare equal", () => {
        assert.equal(normalizeForMatch("git rev-list --count '@{u}...HEAD'"), normalizeForMatch("git rev-list --count @{u}...HEAD"));
    });
...
describe("checkDoc — Side A, every probe is prescribed in a fenced block", () => {
    it("flags a probe dropped from the fenced block even when prose still names it", () => {
        ...
        const mutated = SKILL.replace("git diff --name-only --ignore-all-space", "git diff --stat");
    ...
    it("accepts a probe written with the shell quoting the docs use", () => {
        ...
        assert.ok(probe);
        const quoted = SKILL.replace(probeCommand(probe.args), "git rev-list --left-right --count '@{u}...HEAD'");
        const missing = checkDoc("skills/reconcile.md", quoted).filter((v) => v.kind === "missing-probe");
...
describe("checkDoc — Side B, no fenced block prescribes a retired form", () => {
    it("flags a fenced block that lists a .git-relative marker path", () => {
        const mutated = COMMAND.replace("git stash list", "git stash list\nls .git/MERGE_HEAD 2>/dev/null");
        const retired = checkDoc("commands/reconcile.md", mutated).filter((v) => v.kind === "retired-form");
    ...
    it("flags a fenced block that prescribes branch --show-current", () => {
        ...
        const retired = checkDoc("skills/reconcile.md", mutated).filter((v) => v.kind === "retired-form");
        assert.equal(retired.length, 1);
    });
...
describe("checkDoc — Side C and fail-closed inputs", () => {
    ...
    it("reports a missing repo root rather than clearing it", () => {
        ...
        for (const violation of violations)
            assert.equal(violation.kind, "missing-file");
    });
});
