...
import { DEFAULT_PROTECTED_BRANCHES, GROUND_TRUTH_PROBES, IN_PROGRESS_MARKERS, accountDirty, assessGitState, baselineShifted, classifyHead, classifyInProgress, classifyUpstream, hasBlockers, isProtectedBranch, isSafeRefName, parseRevListCounts, renderResolvedState, verifyPushLanded, } from "../lib/git-state.mjs";
    ...
    it("is portable: no shell, no coreutils, no literal .git/ path (defect 3)", () => {
        for (const probe of GROUND_TRUTH_PROBES) {
            ...
            for (const arg of probe.args) {
                ...
                assert.doesNotMatch(arg, /(^|[^A-Za-z0-9])\.git[\\/]/, `${probe.id} arg "${arg}" hardcodes .git/, which is a FILE in a linked worktree`);
            }
...
describe("isSafeRefName — identity boundary", () => {
    ...
    it("rejects anything that could escape a path or an argv slot", () => {
        for (const bad of [
            ...
            assert.equal(isSafeRefName(bad), false, JSON.stringify(bad));
        }
    });
});
...
describe("verifyPushLanded — three outcomes, never collapsed", () => {
    ...
    it("reports unverified when the local HEAD is not a usable sha", () => {
        assert.equal(verifyPushLanded({ localHead: "", lsRemoteStdout: "", lsRemoteExitCode: 0 }).verdict, "unverified");
    });
    it("reports unverified when ls-remote output carries no parseable sha", () => {
        assert.equal(verifyPushLanded({ localHead: sha, lsRemoteStdout: "garbage\n", lsRemoteExitCode: 0 }).verdict, "unverified");
    });
});
