...
describe("installer", () => {
    ...
    it("normalizes hook command paths for Windows-style homes", () => {
        ...
        try {
            ...
            assert.equal(observeCommands.filter((command) => command.includes("bin/observe.mjs")).length, 1, "legacy observe hook should be replaced, not duplicated");
...
describe("installer - exact hook matching", () => {
    ...
    it("installs the real observe and session hooks even when foreign commands mention their names", () => {
        ...
        const sessionStartCommands = (settings.hooks?.SessionStart || [])
            ...
            .map((hook) => hook.command)
            .filter((command) => Boolean(command));
        assert.equal(preToolCommands.filter((command) => command === observeCommand).length, 1, "real Node observer hook should be installed exactly once");
        assert.equal(sessionStartCommands.filter((command) => command === sessionCommand).length, 1, "real Node session hook should be installed exactly once");
    });
});
describe("installer - cleanup persistence coexistence", () => {
    ...
    it("migrates every legacy observe hook to one Node command", () => {
        ...
        assert.equal(preToolCommands.filter((command) => command === nodeObserveCommand).length, 1, "Node observer hook should exist exactly once");
        assert.equal(preToolCommands.filter((command) => command.includes("observe.sh")).length, 0, "all legacy observe hooks should be removed");
    });
});
...
describe("installer - mixed-entry preservation", () => {
    ...
    it("preserves a foreign command sharing an entry with a broken observe hook", () => {
        ...
        assert.ok(allCommands.some((command) => command.includes("bin/observe.mjs")), "A fresh Node observer hook must be installed");
    });
