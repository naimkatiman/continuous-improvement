...
const SHELL_ENVELOPE_FIELDS = ["head", "upstream", "dirty", "root", "branch"];
const BASH_SKIP = (() => {
        ...
        try {
            ...
            for (const field of SHELL_ENVELOPE_FIELDS) {
                if (field === "root") {
                    ...
                    assert.equal(String(node[field]).replace(/\\/g, "/").toLowerCase().replace(/^\/+/, ""), String(shellEnvelope[field]).replace(/\\/g, "/").toLowerCase().replace(/^\/+/, ""));
                    continue;
                }
