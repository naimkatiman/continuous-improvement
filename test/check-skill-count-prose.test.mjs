...
import { checkProseSurfaces, deriveCounts, ordinal, } from "../bin/check-skill-count-prose.mjs";
...
function goodDocs() {
    return {
        "README.md": "The plugin ships 26 skills — 1 core + 1 featured + 6 tier-1 + 15 tier-2 + 3 always-bundled. Adding a 27th skill.",
        "docs/skills.md": "# the full 26-skill catalog\n1 core + 1 featured + 6 tier-1 + 15 tier-2 + 3 always-bundled = 26 skills. how to add a 27th skill.",
        "docs/skill-use-cases.md": "ships 26 skills (1 core + 1 featured + 6 tier-1 + 15 tier-2 + 3 always-bundled companions).",
...
describe("check-skill-count-prose — deriveCounts", () => {
    it("derives 29 total / 7 tier-1 / 17 tier-2 / next 30 from the live repo", () => {
        const c = deriveCounts(REPO_ROOT);
        assert.equal(c.total, 29, `expected total 29, got ${c.total}`);
        assert.equal(c.core, 1);
        assert.equal(c.featured, 1);
        assert.equal(c.tier1, 7);
...
describe("check-skill-count-prose — checkProseSurfaces", () => {
    ...
    it("passes on the live repo (counts derived from frontmatter)", () => {
        ...
        assert.deepEqual(failures, [], `live repo prose drift: ${JSON.stringify(failures, null, 2)}`);
    });
});
describe("check-skill-count-prose — CLI", () => {
    ...
    it("exits 1 and names the stale surface on a drifted fixture", () => {
        ...
        writeFileSync(join(root, "README.md"), "ships 99 skills — 1 core + 1 featured + 1 tier-1 + 1 tier-2 + 1 always-bundled. Adding a 6th skill.");
        mkdirSync(join(root, "docs"), { recursive: true });
        writeFileSync(join(root, "docs", "skills.md"), "# the full 5-skill catalog\n1 core + 1 featured + 1 tier-1 + 1 tier-2 + 1 always-bundled = 5 skills. how to add a 6th skill.");
        writeFileSync(join(root, "docs", "skill-use-cases.md"), "ships 5 skills (1 core + 1 featured + 1 tier-1 + 1 tier-2 + 1 always-bundled companions).");
