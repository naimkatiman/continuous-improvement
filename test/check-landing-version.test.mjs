...
function setupRepo(version, landing) {
...
function runCheckerExpectingFailure(root) {
    try {
...
describe("check-landing-version CLI", () => {
    it("passes when every landing marker matches package.json", () => {
        const root = setupRepo("3.20.0", landingMarkup({ badge: "3.20.0", hero: "3.20.0", current: "3.20.0", footer: "3.20.0" }));
        try {
    ...
    it("fails when every landing marker is stale", () => {
        const root = setupRepo("3.20.0", landingMarkup({ badge: "3.19.0", hero: "3.19.0", current: "3.19.0", footer: "3.19.0" }));
        try {
    ...
    it("fails when landing markers contain mixed versions", () => {
        const root = setupRepo("3.20.0", landingMarkup({ badge: "3.20.0", hero: "3.19.0", current: "3.20.0", footer: "3.18.0" }));
        try {
            const stderr = runCheckerExpectingFailure(root);
            assert.match(stderr, /FAIL landing-version:/);
...
describe("landing mobile overflow regression", () => {
    it("does not force the hero belt to stay unbroken below 620px", () => {
        ...
        const globalBeltForcesNowrap = /h1\.display\s+\.belt\s*\{[^}]*white-space:\s*nowrap\s*;?[^}]*\}/.test(landing);
        const mobileBeltAllowsWrapping = /h1\.display\s+\.belt\s*\{[^}]*white-space:\s*normal\s*;?[^}]*\}/.test(mobileCss);
        assert.ok(!globalBeltForcesNowrap || mobileBeltAllowsWrapping, "a global nowrap belt must be overridden with white-space: normal at the mobile breakpoint");
    });
});
