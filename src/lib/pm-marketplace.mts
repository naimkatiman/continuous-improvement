...
import type { ClaudeMarketplacePluginEntry } from "./plugin-metadata.mjs";

interface PmPluginJson {
  name: string;
  version: string;
  description: string;
  homepage?: string;
...
export function discoverPmMarketplaceEntries(
  pluginsDir: string,
  ...
  const result: ClaudeMarketplacePluginEntry[] = [];
  for (const dir of pmDirs) {
    ...
    const parsed = JSON.parse(raw) as PmPluginJson;
