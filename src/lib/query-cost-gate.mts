...
export type QueryCostNudgeMode = "on" | "off";

export function resolveQueryCostNudge(raw: string | undefined): QueryCostNudgeMode {
...
const QUERY_PATH_RES: readonly RegExp[] = [
  /\.sql$/i,
  /\.prisma$/i,
  /(^|\/)migrations?\//i,
  /(^|\/)db\//i,
...
export function isQueryPath(filePath: string): boolean {
...
export function changedQueryPaths(files: readonly string[]): string[] {
  return files.filter((file) => isQueryPath(file));
}

export function buildQueryCostReminder(paths: readonly string[]): string {
