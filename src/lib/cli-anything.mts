...
export interface CLIAnythingOptions {
  outputDir?: string;
  verbose?: boolean;
}

export interface ResolvedCLIAnythingOptions {
  outputDir: string;
  verbose: boolean;
}
...
const DEFAULT_OPTIONS: ResolvedCLIAnythingOptions = {
...
export default class CLIAnything {
  public readonly options: ResolvedCLIAnythingOptions;

  constructor(options: CLIAnythingOptions = {}) {
  ...
  async analyzeRepository(repoPath: string): Promise<RepoInfo> {
  ...
  detectProjectType(repoPath: string, packageJson: PackageJson): string {
  ...
  extractExistingCLI(packageJson: PackageJson): ExistingCLI {
    ...
    if (typeof cli.bin === "object") {
      ...
      if (keys.length > 0) cli.mainCommand = keys[0] ?? null;
  ...
  async extractCommands(_repoPath: string, repoInfo: RepoInfo): Promise<Command[]> {
  ...
  generateProjectTypeCommands(repoInfo: RepoInfo): Command[] {
    const commands: Command[] = [];
  ...
  generateCommandsCode(commands: Command[]): string {
  ...
  groupCommands(commands: Command[]): Record<string, Command[]> {
    return commands.reduce<Record<string, Command[]>>((groups, cmd) => {
...
export default {{NAME}}CLI;`,
  ...
  async saveCLI(repoInfo: RepoInfo, cliWrapper: string): Promise<string> {
  ...
  generateScriptDescription(name: string): string {
  ...
  categorizeScript(name: string): CommandCategory {
...
`);
  ...
  const options: CLIAnythingOptions & { template?: string } = {};
