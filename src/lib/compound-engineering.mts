...
export type PhaseName = "brainstorming" | "planning" | "working" | "reviewing";
export type PhaseStatus = "pending" | "active" | "completed";

export interface CompoundEngineeringOptions {
  learningsPath?: string;
  verbose?: boolean;
  workspace?: string;
}

export interface ResolvedCompoundOptions {
  learningsPath: string;
  verbose: boolean;
  workspace: string;
}
...
export interface Plan {
  approach: Idea | string;
  considerations?: string[];
  decisions: string[];
  mitigations: Array<{ mitigation: string; risk: string }>;
  objective: string;
...
export interface Session {
  artifacts: {
    brainstorming?: BrainstormResults;
    planning?: Plan;
    reviewing?: Review;
    working?: WorkLog;
  };
...
const DEFAULT_OPTIONS: ResolvedCompoundOptions = {
...
export default class CompoundEngineering {
  ...
  public learnings: Map<string, Learning> = new Map();

  constructor(options: CompoundEngineeringOptions = {}) {
    this.options = { ...DEFAULT_OPTIONS, ...options };
  }

  async startSession(projectName: string, objective: string): Promise<Session> {
  ...
  async plan(
    brainstormResults: Partial<BrainstormResults> = {},
...
Structure your response as JSON with keys: ideas, constraints, assumptions, risks, opportunities`;
  ...
  performReview(session: Session, workResults: WorkLog, criteria: unknown): Review {
    void criteria;
  ...
  extractLearnings(session: Session, review: Review): Learning[] {
  ...
  async loadLearnings(): Promise<void> {
    try {
      ...
      const parsed = JSON.parse(content) as Learning[];
  ...
  async saveLearnings(newLearnings: Learning[]): Promise<void> {
  ...
  getRelevantLearnings(phase: PhaseName): Learning[] {
  ...
  createMitigations(risks: string[]): Array<{ mitigation: string; risk: string }> {
    ...
    return Array.from(this.learnings.values())
      .filter(
        (l) =>
          l.type === "issue" &&
          l.context &&
          (l.context as { step?: string }).step === step.description,
      )
      .map((l) => l.summary);
  }

  getKnownSolutions(issue: string): string[] {
    return Array.from(this.learnings.values())
      .filter(
        (l) =>
          l.type === "solution" &&
          l.context &&
          (l.context as { issue?: string }).issue === issue,
      )
      .map((l) => l.summary);
  }
  ...
  calculateOutcomeMetrics(workResults: WorkLog): Record<string, number> {
    return {
  ...
  calculateOverallScore(review: Review): number {
    let score = 70;
  ...
  generateRecommendations(review: Review): string[] {
  ...
  extractTags(learning: Partial<Learning>): string[] {
    const tags: string[] = [];
  ...
  getSessionSummary(_sessionId: string): Session | null {
    return this.currentSession;
  }
...
`);
  ...
  const options: CompoundEngineeringOptions = {};
