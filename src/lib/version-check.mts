...
export interface UpdateCache {
  status?: string; // "up-to-date" | "upgrade-available"
  local?: string;
  remote?: string;
  checkedAt?: number; // epoch ms
}

export type UpdateStatus = "unknown" | "up-to-date" | "upgrade-available";

export interface UpdateDecision {
  status: UpdateStatus;
  notice: string | null; // one-line nudge when an upgrade is available, else null
  nextCache: UpdateCache | null; // cache to persist, or null when nothing should change
}
...
export function isThrottled(cache: UpdateCache | null | undefined, now: number): boolean {
  if (!cache || typeof cache.checkedAt !== "number" || !Number.isFinite(cache.checkedAt)) return false;
  const ttl = cache.status === "upgrade-available" ? TTL_UPGRADE_MS : TTL_UP_TO_DATE_MS;
  return now - cache.checkedAt < ttl;
}

function buildNotice(local: string, remote: string): string {
  return (
    `continuous-improvement ${remote} is available (you have ${local}). ` +
...
export function pendingNotice(cache: UpdateCache | null | undefined, local: string): string | null {
  if (!cache || cache.status !== "upgrade-available") return null;
  if (typeof cache.remote !== "string" || !isNewer(cache.remote, local)) return null;
...
export async function fetchLatestNpmVersion(
  ...
  try {
    ...
    if (!version || parseSemver(version) === null) return null;
    return version;
  } catch {
    return null;
  }
