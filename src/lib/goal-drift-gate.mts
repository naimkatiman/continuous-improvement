...
export type GoalDriftMode = "off" | "warn" | "block";
export type GoalDriftAction = "allow" | "warn" | "block";
export type GoalDriftStatus = "on-goal" | "drift" | "no-data" | "no-goal";
...
export interface GoalDriftDecision {
  action: GoalDriftAction;
  status: GoalDriftStatus;
  score: number;
  reason: string;
}

export function evaluateGoalDrift(input: GoalDriftInput): GoalDriftDecision {
  ...
  const reason =
    `Session has DRIFTed from the stated goal (${report.matching}/${report.total} recent actions on-goal, ${pct}%). ` +
