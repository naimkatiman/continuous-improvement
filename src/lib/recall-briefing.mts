...
import type { RecallHit } from "./recall-index.mjs";

export interface BriefingParams {
  /** True only when the operator opted in (the hook reads the env flag). */
  enabled: boolean;
  /** True if this session already received a briefing — brief at most once. */
  alreadyBriefed: boolean;
  /** The first user prompt of the session. */
  prompt: string;
  /** Ranked recall hits for the prompt (caller queries the index). */
  hits: RecallHit[];
  /** Max hits to surface (default 3). */
  maxHits?: number;
  /** Minimum trimmed prompt length before briefing (default 12) — a noise floor. */
...
export function formatBriefing(hits: RecallHit[]): string {
  const lines: string[] = [
    "<system-reminder>",
 ...
 */
export function decideBriefing(params: BriefingParams): BriefingDecision {
  const none: BriefingDecision = { shouldBrief: false, reminder: null };
  if (!params || params.enabled !== true) return none;
  if (params.alreadyBriefed === true) return none;

  const prompt = typeof params.prompt === "string" ? params.prompt : "";
  const minLen =
    typeof params.minPromptLength === "number" && params.minPromptLength >= 0
      ? params.minPromptLength
      : DEFAULT_MIN_PROMPT_LENGTH;
  if (prompt.trim().length < minLen) return none;

  const maxHits =
    typeof params.maxHits === "number" && params.maxHits > 0
