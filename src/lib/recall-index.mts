...
export function redactSecrets(text: string): string {
  return text
    ...
    .replace(
      /\b([A-Za-z][A-Za-z0-9_]*(?:KEY|SECRET|TOKEN|PASSWORD|PASSWD|PWD))(\s*[:=]\s*)(\S+)/gi,
      "$1$2<redacted>",
    )
...
function docText(obs: RecallObservation): string {
...
export function buildIndex(observations: RecallObservation[]): RecallIndex {
...
function scoreDoc(index: RecallIndex, doc: IndexedDoc, queryTerms: string[]): number {
  let score = 0;
...
export function parseSince(since: string, now: number): number | null {
...
function obsSession(obs: RecallObservation): string {
 ...
 */
export function query(index: RecallIndex, queryString: string, opts: RecallOpts = {}): RecallHit[] {
...
export function formatRecallHits(hits: RecallHit[], queryString: string): string {
  if (hits.length === 0) {
    return `No past observations matched "${queryString}". Try broader or different terms — recall is lexical, not semantic.`;
  }

  const lines: string[] = [`## Recall: "${queryString}"`, "", `${hits.length} match(es), most relevant first:`, ""];
