...
function runObserve(payload: string, extraEnv: Record<string, string> = {}) {
...
function findProjectDir(home: string): string {
...
function readObservations(home: string): Observation[] {
...
describe("observe.mjs (Node observer entrypoint)", () => {
  ...
  it("writes C:/ and c:/ project dirs into one hash bucket", () => {
    ...
    try {
      ...
      const project = JSON.parse(readFileSync(join(projectDir, "project.json"), "utf8")) as { root: string };
