...
interface Ladder {
  build: PhaseEntry;
  typecheck: PhaseEntry;
  lint: PhaseEntry;
  test: PhaseEntry;
  security: PhaseEntry;
  deploy_receipt: PhaseEntry;
  synthetic_checks: PhaseEntry;
}

function runJson(cwd: string): { exitCode: number; ladder: Ladder | null; stderr: string } {
  ...
  if (result.stdout) {
    try {
      ladder = JSON.parse(result.stdout) as Ladder;
    } catch {
...
describe("scripts/resolve-verify-ladder.mjs", () => {
  it("returns ask-operator for every field in an empty directory", () => {
    ...
    try {
      ...
      for (const phase of ["build", "typecheck", "lint", "test", "security", "deploy_receipt"] as const) {
  ...
  it("reads .claude/verify-ladder.json manifest first when present", () => {
    ...
    try {
      ...
      writeFileSync(
        join(tmp, ".claude", "verify-ladder.json"),
        JSON.stringify({
          build: "make all",
