...
function seedObservations(projectRoot: string, inputs: string[]): void {
  ...
  const rows = inputs
    .map((input) =>
      JSON.stringify({ ts: "2026-06-03T12:00:00Z", tool: "Edit", input_summary: input, output_summary: "" }),
    )
    .join("\n");
...
describe("goal-drift-stop hook", () => {
  beforeEach(() => {
    ...
    writeFileSync(
      transcriptPath,
      JSON.stringify({ type: "assistant", message: { content: [{ type: "text", text: LONG_TEXT }] } }) + "\n",
    );
  });
