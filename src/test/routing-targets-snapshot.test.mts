...
describe("check-routing-targets — resolveVendoredSkillPath", () => {
  it("returns the snapshot path for a vendored prefix", () => {
    assert.equal(
      resolveVendoredSkillPath("oh-my-claudecode:ultragoal"),
      "third-party/oh-my-claudecode/skills/ultragoal",
    );
  });

  it("uses the mapped directory, not the prefix, when they differ", () => {
    assert.equal(
      resolveVendoredSkillPath("agent-skills:context-engineering"),
      "third-party/addy-agent-skills/skills/context-engineering",
    );
  });
  ...
  it("keeps a colon that appears inside the skill name", () => {
    assert.equal(
      resolveVendoredSkillPath("superpowers:a:b"),
      "third-party/superpowers/skills/a:b",
    );
  });
});
...
describe("check-routing-targets — against the real repo", () => {
  it("every declared vendored companion exists in its snapshot", () => {
    ...
    assert.deepEqual(
      result.missingVendored.map((m) => m.target),
      [],
      "a declared oh-my-claudecode:/superpowers:/agent-skills:/ruflo-swarm: target has no directory in the vendored snapshot",
    );
  });
