...
import {
  ...
  findRemovedSkills,
  findStaleReferences,
} from "../bin/refresh-third-party.mjs";
...
function makeSkills(rel: string, names: string[]): string {
...
describe("refresh-third-party — findStaleReferences", () => {
  ...
  it("never scans the vendored snapshots themselves", () => {
    assert.ok(
      REFERENCE_SOURCES.every((p) => !p.startsWith("third-party")),
      "scanning third-party/ would match the snapshot being replaced",
    );
  });
