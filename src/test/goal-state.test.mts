...
function obs(partial: Partial<GoalObservation>): GoalObservation {
...
describe("scoreObservations", () => {
  ...
  it("honors a custom window", () => {
    const observations = Array.from({ length: 50 }, (_, i) =>
      obs({ tool: "Read", input_summary: i < 45 ? "unrelated.ts" : "src/auth/login.ts" }),
    );
...
describe("formatDriftReport", () => {
  it("renders an on-goal report without the steer-back nudge", () => {
    const report = scoreObservations(
      [obs({ tool: "Edit", input_summary: "src/auth/login.ts" })],
      { prose: "", keywords: ["login"], paths: [], forbidden: [] },
    );
  ...
  it("renders a drift report with the steer-back nudge and mismatches", () => {
    const report = scoreObservations(
      [obs({ tool: "Edit", input_summary: "src/other/thing.ts" })],
      { prose: "", keywords: ["login"], paths: [], forbidden: [] },
    );
    const out = formatDriftReport(report);
...
describe("scoreObservations — out-of-range window (item 3)", () => {
  ...
  for (const bad of [0, -5, 2.5, Number.NaN, Number.POSITIVE_INFINITY]) {
    it(`rejects an explicit out-of-range window (${bad}) instead of silently defaulting`, () => {
      assert.throws(
        () => scoreObservations(one, goal, { window: bad }),
        /window/i,
        `window=${bad} must be rejected, not coerced to the default`,
      );
    });
  }
});
