...
interface BackfillSummary {
  ...
  thin: number;
  rich: number;
  projects: Record<string, { total: number; thin: number; rich: number }>;
}
...
function instinctsDirOf(home: string): string {
  return join(home, ".claude", "instincts");
}

function makeProject(home: string, hash: string, rows: object[]): string {
  ...
  writeFileSync(
    ...
    JSON.stringify({ id: hash, name: `proj-${hash}`, root: `/tmp/${hash}`, created_at: "2026-05-05T00:00:00Z" }) + "\n",
    "utf8"
  );
  return file;
}

function runBackfill(home: string, extraArgs: string[] = []) {
...
function readJsonl(file: string): BackfillRow[] {
  if (!existsSync(file)) return [];
  return readFileSync(file, "utf8")
    .trim()
    .split("\n")
    .filter((line) => line.length > 0)
    .map((line) => JSON.parse(line) as BackfillRow);
}
...
describe("backfill.mjs subcommand", () => {
  ...
  it("emits a summary.json digest with per-project counts", () => {
    ...
    try {
      ...
      const summary = JSON.parse(readFileSync(summaryPath, "utf8")) as BackfillSummary;
