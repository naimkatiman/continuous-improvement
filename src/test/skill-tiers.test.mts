...
describe("parseSkillFrontmatter", () => {
  ...
  it("collapses folded block scalars (description: >) into one line", () => {
    ...
    assert.equal(
      out.description,
      "File-based memory system using PARA. Three layers: knowledge graph, daily notes, and tacit knowledge.",
    );
  });
...
describe("groupSkillsByTier", () => {
  it("groups skills into the correct buckets and sorts each bucket", () => {
    ...
    assert.deepEqual(
      ...
      ["gateguard", "verification-loop"], // alphabetically sorted
    );
    assert.deepEqual(groups["2"], []);
    assert.deepEqual(
      groups.companion.map((s) => s.name),
      ["ralph"],
    );
    assert.deepEqual(
      groups.unknown.map((s) => s.name),
      ["mystery"],
    );
  });
});
...
describe("renderBundledSkillsReadme", () => {
  ...
  it("marks the featured tier with a star, but not the core tier", () => {
    ...
    assert.doesNotMatch(
      out,
      /- `continuous-improvement` ⭐/,
      "core skill must not carry the star (it's not a companion)",
    );
  });
