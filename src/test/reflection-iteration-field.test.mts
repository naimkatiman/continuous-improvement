...
describe("Reflection block 'Iteration — Next best recommendations' field", () => {
  for (const mirror of MIRRORS) {
    describe(mirror.path, () => {
      ...
      it(`contains the Iteration field in ${mirror.section}`, () => {
        assert.ok(
          content.includes(ITERATION_FIELD),
          `${mirror.path} is missing the Iteration field. Each Reflection block must include the literal:\n  ${ITERATION_FIELD}\nSee SKILL.md Law 5 for the canonical template.`
        );
      });
      ...
      it("contains the 3-item ranked structure (1./2./3.)", () => {
        ...
        assert.match(
          tail,
          /\b1\./,
          `${mirror.path} Iteration block must include item "1."`
        );
        assert.match(
          tail,
          /\b2\./,
          `${mirror.path} Iteration block must include item "2."`
        );
        assert.match(
          tail,
          /\b3\./,
          `${mirror.path} Iteration block must include item "3."`
        );
      });
    });
  }
  ...
  it("legacy single-item phrase 'Iteration — Next best recommendation:' is gone (no trailing 's')", () => {
    ...
    for (const mirror of MIRRORS) {
      ...
      assert.ok(
        !legacyPhrase.test(content),
        `${mirror.path} still contains the legacy single-item phrase. Update to plural ranked-top-3 form.`
      );
    }
  });
});
