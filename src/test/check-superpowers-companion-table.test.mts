...
import {
  SECTION_HEADER,
  extractCompanionTableRows,
  extractSkillRefs,
  checkSuperpowersCompanionTable,
} from "../bin/check-superpowers-companion-table.mjs";
...
function writeCommand(root: string, tableRows: string[]): void {
  const body = [
    "# /superpowers",
    "",
    SECTION_HEADER,
...
function writeVendoredSkill(root: string, snapshot: string, name: string): void {
  const dir = join(root, "third-party", snapshot, "skills", name);
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "SKILL.md"), `# ${name}\n`);
}

function writeBundledSkill(root: string, name: string): void {
...
describe("check-superpowers-companion-table — parser", () => {
  ...
  it("throws when the companion-table heading is missing", () => {
    assert.throws(
      () => extractCompanionTableRows("# /superpowers\n\nNo table here.\n"),
...
describe("check-superpowers-companion-table — the three deleted skills", () => {
  it("flags ultrawork, ultraqa and deep-dive when the snapshot no longer ships them", () => {
    ...
    try {
      ...
      assert.ok(
        result.missing.every((m) => m.expectedPath.includes("oh-my-claudecode")),
      );
    } finally {
...
describe("check-superpowers-companion-table — live repo", () => {
  it("every skill named in commands/superpowers.md's companion table exists in its snapshot", () => {
    ...
    assert.deepEqual(
      ...
      [],
      `companion table names skills the snapshot does not ship: ${JSON.stringify(result.missing, null, 2)}`,
    );
  });
});
