...
describe("Unified Continuous Improvement Plugin", () => {
  ...
  test("should derive integrated timeline dependencies and milestones", () => {
    ...
    assert.ok(
      timeline.dependencies.some((dependency) =>
        String(dependency).includes("depends on"),
      ),
    );
    assert.ok(
      timeline.milestones.some((milestone) =>
        JSON.stringify(milestone).includes("Mobile App Launch"),
      ),
    );
  });
  ...
  test("should generate workflow summary and detailed report from project context", async () => {
    ...
    assert.match(report.executiveSummary as string, /SummaryTest/);
    assert.equal(
      (report.detailedAnalysis as { progress: { percentage: number } }).progress.percentage,
      100,
    );
