...
interface Scan {
  ...
  claude_md: ClaudeMdEntry[];
  sources: { observations: string; memory_dir: string; claude_md: string };
}

function runJson(args: string[]): { exitCode: number; scan: Scan | null; stderr: string; stdout: string } {
  const result = spawnSync("node", [SCRIPT, "--json", ...args], { encoding: "utf8" });
  let scan: Scan | null = null;
  if (result.stdout) {
    try {
      scan = JSON.parse(result.stdout) as Scan;
    } catch {
      // leave null
    }
  }
...
function runPretty(args: string[]): { exitCode: number; stdout: string; stderr: string } {
...
function makeTempScope(): { dir: string; obsPath: string; memDir: string; claudeMdPath: string } {
...
function explicitArgs(scope: { obsPath: string; memDir: string; claudeMdPath: string }): string[] {
  return [
...
describe("scripts/scan-past-mistakes.mjs", () => {
  ...
  it("default mode (no --json) prints a human-readable summary", () => {
    ...
    try {
      mkdirSync(scope.memDir, { recursive: true });
      writeFileSync(
        join(scope.memDir, "feedback_alpha.md"),
