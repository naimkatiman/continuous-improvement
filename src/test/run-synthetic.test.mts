...
function setupFixture(files: FixtureFile[]): string {
  ...
  it("--json emits a structured report instead of human text", () => {
    ...
    try {
      ...
      const parsed = JSON.parse(stdout) as {
        dir: string;
        checks: Array<{ file: string; exitCode: number; status: string }>;
        summary: { pass: number; fail: number; configError: number; skipped: number; total: number };
      };
...
describe("run-synthetic — timeout", () => {
  it("--timeout kills a check that exceeds the wall-clock cap", () => {
    ...
    try {
      ...
      const parsed = JSON.parse(stdout) as {
        checks: Array<{ file: string; status: string; exitCode: number }>;
      };
