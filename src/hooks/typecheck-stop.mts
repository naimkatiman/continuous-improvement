...
import {
  decideTypecheckAction,
  formatTypecheckReason,
  hasChangedTsFile,
  parseChangedFiles,
  pickTypecheckKind,
  resolveTypecheckMode,
  type TypecheckMode,
} from "../lib/typecheck-gate.mjs";
...
function hasNpmTypecheckScript(root: string): boolean {
  try {
...
function localTscPath(root: string): string | null {
...
function runTypecheck(root: string): RunResult {
  ...
  const result =
    kind === "npm"
      ...
      : spawnSync(process.execPath, [tscPath as string, "--noEmit"], {
          cwd: root,
          encoding: "utf8",
          timeout: SPAWN_TIMEOUT_MS,
        });
  ...
  if (result.error || typeof result.status !== "number") return { ran: false, rc: 0, output: "" };
  return { ran: true, rc: result.status, output: `${result.stdout ?? ""}${result.stderr ?? ""}` };
}

function main(): void {
  const mode: TypecheckMode = resolveTypecheckMode(process.env.CLAUDE_TYPECHECK_GATE);
  if (mode === "off") return;

  const root = resolveProjectRoot();
  if (!root || !existsSync(join(root, "tsconfig.json"))) return; // not a TS project

  if (!hasChangedTsFile(collectChangedFiles(root))) return; // nothing TS changed — near-zero cost
