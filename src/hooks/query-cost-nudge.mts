...
function markerKey(sessionId: string | undefined): string {
...
function main(): void {
  if (resolveQueryCostNudge(process.env.CLAUDE_QUERY_COST_NUDGE) === "off") return;
  ...
  if (root === "global") return; // no repo → nothing to diff
  ...
  try {
    if (existsSync(marker)) return; // already nudged this session
