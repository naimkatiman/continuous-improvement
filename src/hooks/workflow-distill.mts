...
function readNudgedKeys(instinctsProjectDir: string): Set<string> {
  const file = join(instinctsProjectDir, "workflow-distill-state.json");
  try {
    if (!existsSync(file)) return new Set();
    const parsed = safeJsonParse(readFileSync(file, "utf8")) as { nudged?: unknown } | null;
...
function persistNudged(instinctsProjectDir: string, keys: Set<string>): void {
  try {
    mkdirSync(instinctsProjectDir, { recursive: true });
    writeFileSync(
      join(instinctsProjectDir, "workflow-distill-state.json"),
      JSON.stringify({ ts: new Date().toISOString(), nudged: [...keys] }) + "\n",
      "utf8",
    );
  } catch {
...
function main(): void {
  ...
  if (!run) return; // fail closed: no verified run, nothing to nudge

  const runKey = `${run.name}::${run.verifyCommand}`;
  const nudged = readNudgedKeys(instinctsProjectDir);
  if (nudged.has(runKey)) return; // already nudged this run — stay quiet

  process.stderr.write(
    `[continuous-improvement] workflow-distill: verified workflow run "${run.name}" detected — ` +
