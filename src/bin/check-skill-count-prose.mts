...
export function deriveCounts(repoRoot: string): SkillCounts {
...
export type ProseCheck = "breakdown" | "total-skills" | "total-catalog" | "next-ordinal";

/** The exact substring a surface must contain for the given check kind. */
export function expectedFor(check: ProseCheck, c: SkillCounts): string {
  switch (check) {
    case "breakdown":
...
export const PROSE_SURFACES: ProseSurface[] = [
 ...
 */
export function checkProseSurfaces(
  repoRoot: string,
  counts?: SkillCounts,
  surfaces: ProseSurface[] = PROSE_SURFACES,
): ProseFailure[] {
...
function main(): void {
  ...
  if (failures.length === 0) {
    console.log(
      `OK skill-count-prose: all ${PROSE_SURFACES.length} doc surface(s) state the derived count (total=${counts.total}: ${expectedFor("breakdown", counts)}).`,
    );
    exit(0);
  }
  console.error(
    `FAIL skill-count-prose: ${failures.length} doc prose claim(s) do not match the bundle-derived count of ${counts.total}.`,
  );
  console.error(
    `Source of truth: skills/ frontmatter → ${expectedFor("breakdown", counts)} = ${counts.total} skills; next skill is the ${ordinal(counts.next)}.`,
  );
  console.error("");
  ...
  console.error("");
  console.error(
    "Fix: bump the prose in the named file(s) to the derived count. When a skill is genuinely added or retired, this lint enumerates every human-facing surface that needs the same change.",
  );
  exit(1);
}
