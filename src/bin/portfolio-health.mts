...
import { auditWorkflows, type WorkflowFile } from "./audit-actions.mjs";
...
function countProofFiles(localPath: string, dirs: string[]): number {
  let count = 0;
  for (const dir of dirs) {
    const full = join(localPath, dir);
    if (!existsSync(full)) continue;
    for (const entry of readdirSync(full)) {
      if (entry.endsWith(".md") && !TEMPLATE_NAME.test(entry)) count++;
    }
  }
  return count;
}

function readWorkflowFiles(localPath: string): WorkflowFile[] {
...
function daysSinceLastCommit(localPath: string, nowMs: number): number | null {
  try {
...
export function collectSignals(localPath: string, opts: { nowMs?: number } = {}): RepoSignals {
...
function freshnessPoints(days: number | null): number {
  if (days === null) return 0;
  if (days <= FRESH_DAYS) return WEIGHT_FRESHNESS;
  if (days <= STALE_DAYS) return WEIGHT_FRESHNESS / 2;
  return 0;
}
 ...
 */
export function scoreRepo(s: RepoSignals): { score: number; topRisk: string; nextAction: string } {
  ...
  const experiments = Math.round(
    (WEIGHT_EXPERIMENTS * Math.min(s.experimentCount, EXPERIMENT_FULL_CREDIT)) / EXPERIMENT_FULL_CREDIT,
  );
  ...
  const gaps: Array<{ lost: number; risk: string; action: string }> = [
    {
...
export function loadConfig(configPath: string): RepoEntry[] {
  ...
  for (let i = 0; i < data.length; i++) {
    const e = data[i] as Partial<RepoEntry>;
    const missing: string[] = [];
    if (typeof e.repo !== "string" || e.repo === "") missing.push("repo");
    if (typeof e.lane !== "string" || e.lane === "") missing.push("lane");
    if (typeof e.localPath !== "string" || e.localPath === "") missing.push("localPath");
    if (typeof e.active !== "boolean") missing.push("active");
    if (missing.length > 0) {
      throw new Error(`portfolio registry entry ${i} has missing/invalid field(s): ${missing.join(", ")}`);
    }
    entries.push({ repo: e.repo as string, lane: e.lane as string, localPath: e.localPath as string, active: e.active as boolean });
  }
  return entries;
}
 ...
 */
export function buildRows(entries: RepoEntry[], opts: { nowMs?: number } = {}): RepoRow[] {
  const rows: RepoRow[] = entries.map((entry) => {
...
export function renderReport(rows: RepoRow[], generatedAt: string): string {
  const out: string[] = [];
