...
const DOCS = ["skills/reconcile.md", "commands/reconcile.md"] as const;

const RETIRED_FORMS: readonly { pattern: RegExp; label: string; why: string }[] = [
  {
    pattern: /\bls\s+[^\n`]*\.git\//,
    label: "ls .git/<marker>",
...
const REQUIRED_SUBSTRINGS: readonly { needle: string; why: string }[] = [
  {
...
export interface ParityViolation {
  doc: string;
  kind: "missing-file" | "empty" | "missing-probe" | "retired-form" | "missing-substring";
  detail: string;
}
...
export function probeCommand(args: readonly string[]): string {
...
export function checkDoc(doc: string, body: string | null): ParityViolation[] {
  if (body === null) return [{ doc, kind: "missing-file", detail: "file does not exist" }];
  if (body.trim().length === 0) return [{ doc, kind: "empty", detail: "file is empty" }];
...
export function checkRepo(repoRoot: string): ParityViolation[] {
...
function main(): void {
  ...
  if (violations.length === 0) {
    console.log(
      `OK reconcile-parity: ${DOCS.length} doc(s) document all ${GROUND_TRUTH_PROBES.length} ground-truth probe(s), no retired forms.`,
    );
    return;
  }
  ...
  console.error("");
  console.error(
    "Fix: GROUND_TRUTH_PROBES in src/lib/git-state.mts is the source of truth. Update the",
  );
  console.error(
    "doc prose to match it (or change the probe set there first), then `npm run build` so the",
  );
