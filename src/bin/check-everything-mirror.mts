...
const EXCLUDE_DIRS = new Set<string>(["skills", ".claude-plugin"]);

interface MirrorPair {
  relPath: string; // relative-to-plugin-root, forward-slash normalized
  pluginPath: string; // absolute
  flatPath: string; // absolute
}

export interface MirrorDrift {
  relPath: string;
  reason: "missing-flat" | "content-drift";
  pluginPath: string;
  flatPath: string;
  pluginBytes?: number;
  flatBytes?: number;
...
export function discoverMirrorPairs(repoRoot: string): MirrorPair[] {
  ...
  function walk(dir: string): void {
    ...
    for (const name of entries) {
      ...
      if (stat.isDirectory()) {
        if (EXCLUDE_DIRS.has(relFromPlugin)) continue;
        walk(full);
        continue;
      }
      if (!stat.isFile()) continue;
      if (EXCLUDE_FILES.has(relFromPlugin)) continue;
      out.push({
...
export function checkMirrorPairs(repoRoot: string): MirrorDrift[] {
  const pairs = discoverMirrorPairs(repoRoot);
  const drifts: MirrorDrift[] = [];
  for (const pair of pairs) {
    let flatContent: Buffer;
    try {
...
function main(): void {
  ...
  if (drifts.length === 0) {
    console.log(
      `OK everything-mirror: all ${pairs.length} mirrored file(s) match their flat-tree sibling.`,
    );
    exit(0);
  }
  console.error(
    `FAIL everything-mirror: ${drifts.length} drift(s) detected across ${pairs.length} mirrored file(s).\n`,
  );
  for (const d of drifts) {
    if (d.reason === "missing-flat") {
      console.error(
        `  - ${d.relPath}: plugin copy exists but flat sibling is missing.`,
      );
      console.error(`      plugin: ${d.pluginPath}`);
      console.error(`      flat:   ${d.flatPath} (NOT FOUND)`);
    } else {
      console.error(
        `  - ${d.relPath}: byte drift between plugin and flat copy (${d.pluginBytes} vs ${d.flatBytes} bytes).`,
      );
      console.error(`      plugin: ${d.pluginPath}`);
