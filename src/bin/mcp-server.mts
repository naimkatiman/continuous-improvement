...
function readDistillObservations(projectHash: string): DistillObservation[] {
...
function handleTool(name: string, params: Record<string, unknown>): ToolResult {
  ...
  switch (name) {
    ...
    case "ci_goal_check": {
      ...
      if (!goal) {
        return text(
          `Found ${goalSource} but it has no '## Goal' section. Add one — ci_plan_init seeds it. Optional '## Goal Keywords' and '## Goal Scope' sections sharpen the drift signal.`
        );
      }
    ...
    case "ci_distill_promote": {
      ...
      return text(
        `Promoted **${promoted.id}** to a project instinct at confidence ${promoted.confidence} (SUGGEST tier). The draft has been consumed.`
      );
    }
