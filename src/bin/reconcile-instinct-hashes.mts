...
export function discoverGroups(instinctsRoot: string): AliasGroup[] {
  ...
  for (const name of entries) {
    ...
    try {
      project = JSON.parse(readFileSync(projectPath, "utf8")) as ProjectJson;
    } catch {
      continue;
    }
  ...
  return [...groups.values()].filter((group) =>
    group.members.some((member) => member.hash !== group.canonicalHash && member.rows > 0),
  );
}
...
export function mergeJsonl(sources: string[]): string {
  const rows: string[] = [];
...
export function selectGroups(groups: AliasGroup[], only: string[]): AliasGroup[] {
  if (only.length === 0) return groups;
  const wanted = new Set(only);
  return groups.filter((group) =>
    wanted.has(group.canonicalHash) || group.members.some((member) => wanted.has(member.hash)),
  );
}

export function applyGroup(instinctsRoot: string, group: AliasGroup): { copiedRows: number; aliases: string[] } {
  ...
  if (existsSync(destProject)) {
    try {
      const existing = JSON.parse(readFileSync(destProject, "utf8")) as ProjectJson;
  ...
  for (const member of group.members) {
    if (member.hash === group.canonicalHash) continue;
...
function printPlan(groups: AliasGroup[]): void {
  if (groups.length === 0) {
...
function parseOnly(args: string[]): string[] {
  const only: string[] = [];
