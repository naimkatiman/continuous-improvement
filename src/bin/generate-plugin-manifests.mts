...
async function writeBundledSkills(): Promise<void> {
  ...
  const FEATURED_SKILL = "proceed-with-claude-recommendation";
  const renderSkillLine = (name: string): string =>
    name === FEATURED_SKILL
      ? `- \`${name}\` ⭐ — featured companion for the 7 Laws (executes recommendation lists end-to-end with per-item verification)`
      : `- \`${name}\``;
  const skillsReadme = [
    ...
    "",
    "## Included skills",
    ...bundledSkillNames.map(renderSkillLine),
    "",
  ].join("\n");
