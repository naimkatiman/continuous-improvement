...
interface LandingVersionResult {
  ok: boolean;
...
export function checkLandingVersion(repoRoot: string): LandingVersionResult {
  ...
  const markers: LandingMarkers = {
    badge: firstMatch(
      landing,
      new RegExp(`<span\\b[^>]*class=["'][^"']*\\bver\\b[^"']*["'][^>]*>\\s*v${VERSION_RE}\\s*</span>`, "i"),
    ),
    hero: firstMatch(
      landing,
      new RegExp(`<span\\b[^>]*class=["'][^"']*\\bkicker\\b[^"']*["'][^>]*>[\\s\\S]*?\\bREV\\s+${VERSION_RE}[\\s\\S]*?</span>`, "i"),
    ),
    current: firstMatch(
      landing,
      new RegExp(`<span\\b[^>]*class=["'][^"']*\\bv\\b[^"']*["'][^>]*>\\s*v${VERSION_RE}\\s*</span>\\s*<span\\b[^>]*class=["'][^"']*\\bk\\b[^"']*["'][^>]*>\\s*Current rev\\s*</span>`, "i"),
    ),
    footer: firstMatch(
      landing,
      new RegExp(`<span\\b[^>]*class=["'][^"']*\\bfoot-doc\\b[^"']*["'][^>]*>[\\s\\S]*?\\bREV\\s+${VERSION_RE}[\\s\\S]*?</span>`, "i"),
    ),
  };
  ...
  const landingVersions = [...new Set(Object.values(markers).filter((value): value is string => value !== null))];
