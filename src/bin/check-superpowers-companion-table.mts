...
export interface CompanionTableRow {
  source: string;
  lives: string;
  snapshotDir: string | null;
  kind: "vendored" | "bundled" | "other";
  skills: string[];
}

export interface CompanionSkillRef {
  skill: string;
  source: string;
  kind: "vendored" | "bundled";
  expectedPath: string;
}

export interface MissingCompanionSkill {
  skill: string;
  source: string;
  expectedPath: string;
}
...
export interface CheckResult {
  rows: CompanionTableRow[];
  refs: CompanionSkillRef[];
  missing: MissingCompanionSkill[];
}
...
export function extractSkillRefs(examplesCell: string): string[] {
...
function classifyRow(lives: string): Pick<CompanionTableRow, "snapshotDir" | "kind"> {
  const snap = SNAPSHOT_RE.exec(lives);
  if (snap) return { snapshotDir: snap[1], kind: "vendored" };
  if (/\bbundled\b/i.test(lives)) return { snapshotDir: null, kind: "bundled" };
  return { snapshotDir: null, kind: "other" };
}

export function extractCompanionTableRows(markdown: string): CompanionTableRow[] {
...
function expectedPath(row: CompanionTableRow, skill: string): string {
  if (row.kind === "bundled") return join("skills", `${skill}.md`);
  return join("third-party", row.snapshotDir ?? "unknown", "skills", skill);
}

export function checkSuperpowersCompanionTable(repoRoot: string): CheckResult {
  const markdown = readFileSync(join(repoRoot, COMMAND_PATH), "utf8");
  const rows = extractCompanionTableRows(markdown);
  const refs: CompanionSkillRef[] = [];
  const missing: MissingCompanionSkill[] = [];
...
function main(): void {
  ...
  if (result.missing.length === 0) {
    console.log(
      `OK superpowers-companion-table: all ${result.refs.length} companion-table skill(s) exist in their snapshots.`,
    );
    exit(0);
  }

  console.error(
    `FAIL superpowers-companion-table: ${result.missing.length} named skill(s) in ${COMMAND_PATH} do not exist in the vendored snapshot.\n`,
  );
