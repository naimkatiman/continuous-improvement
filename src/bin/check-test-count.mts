...
const STRIP_RE =
  /<span\b[^>]*class=["'][^"']*\bv\b[^"']*["'][^>]*>\s*([\d,]+)\s*<\/span>\s*<span\b[^>]*class=["'][^"']*\bk\b[^"']*["'][^>]*>\s*Tests in the suite\s*<\/span>/gi;
...
export interface TestCountClaims {
  strip: number[];
  ladder: number[];
}

export function extractClaims(html: string): TestCountClaims {
...
export function parseActualCount(runOutput: string): number | null {
  const matches = [...runOutput.matchAll(/^\W*tests\s+(\d+)\s*$/gim)];
  if (matches.length === 0) return null;
  return Number(matches[matches.length - 1][1]);
}

export function findViolations(claims: TestCountClaims, actual: number | null): string[] {
  const violations: string[] = [];
  if (claims.strip.length === 0) {
    violations.push(
      `the spec strip states no test count (expected a <span class="v">N</span> cell labelled "Tests in the suite").`,
    );
  }
  if (claims.ladder.length === 0) {
    violations.push(
      `the verification-loop ladder states no test count (expected a "tests      PASS  N/N" line).`,
    );
  }
...
export interface ApplyTestCountResult {
  html: string;
  changed: boolean;
}
 ...
 */
export function applyTestCount(html: string, actual: number): ApplyTestCountResult {
...
function main(): void {
  ...
  if (actualPath) {
    ...
    if (actual === null) {
      console.error(
        `FAIL test-count: ${actualPath} has no "tests N" summary line, so the claim cannot be checked against a real run.`,
      );
      exit(1);
    }
  }
  ...
  if (write) {
    ...
    let result: ApplyTestCountResult;
    try {
  ...
  console.error("");
  console.error(
    `Fix: run the suite, take the "tests N" total, and update both surfaces in ${LANDING} (the spec strip and the verification-loop panel).`,
  );
  exit(1);
}
